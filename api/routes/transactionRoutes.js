...
router.get('/', async (req, res) => {
    try {
        const transactions = await Transaction.find().populate('store').populate('user');
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            const transaction = new Transaction({ store, user, amount: Number(amount), type });
            ...
            if (type === 'buy') {
                ...
                if (storeExists.amount < Number(amount)) {
                    return res.status(400).send({ error: 'Insufficient stock in the store' });
                }
                storeExists.amount -= Number(amount);
            }
